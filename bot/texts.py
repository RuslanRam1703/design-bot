...
def supplement_ack(lead_id: int) -> str:
    """Подтверждение дополнения к уже существующей заявке.

    Отдельно от lead_received_ack: там шапка про НОВУЮ заявку («Заявка
    отправлена», услуга, предварительная цена), а здесь заявка уже
    существует и ничего из этого повторять не нужно."""
    return f"Дополнение к заявке №{lead_id} получено 🙌\n\nЯ учту его и свяжусь с вами."


def supplement_ack_ask_file(lead_id: int) -> str:
    """То же дополнение, но клиент отметил «пришлю файл».

    Формулировка про файл намеренно совпадает по смыслу с
    lead_ack_ask_file: клиент попадает в один и тот же чат и должен
    получить одну и ту же инструкцию, независимо от того, из какого
    экрана Mini App он пришёл."""
    return (
        f"Дополнение к заявке №{lead_id} получено 🙌\n\n"
        "Вы отметили, что пришлёте файл — отправьте его следующим сообщением, я его посмотрю."
    )
